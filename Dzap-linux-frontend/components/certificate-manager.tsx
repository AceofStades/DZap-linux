...
import { Search, Download, FileText, Shield, Calendar, QrCode, Filter, Eye } from "lucide-react"
...
export function CertificateManager() {
  ...
  const handleDownloadCertificate = (certificate: Certificate, format: "pdf" | "json") => {
    if (format === "json") {
      const certData = {
      ...
      console.log("Generating PDF certificate for:", certificate.certificateId)
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">View and manage data destruction certificates</p>
      </div>
      ...
      <Card className="component-border component-border-hover">
        <CardHeader>
          ...
          <CardDescription>Data destruction certificates and verification records</CardDescription>
        </CardHeader>
        <CardContent>
              ...
              <TableBody>
                {filteredCertificates.map((cert) => (
                  <TableRow key={cert.id} className="hover:bg-muted/50">
                    <TableCell>
                      <div>
                        <div className="font-medium font-mono text-sm text-foreground">{cert.certificateId}</div>
                        <div className="text-xs text-muted-foreground">
                          {cert.signatureValid ? "Signature Valid" : "Signature Invalid"}
                        </div>
                      </div>
                    </TableCell>
                    <TableCell>
                      <div>
                        <div className="font-medium text-foreground">{cert.deviceName}</div>
                        <div className="text-sm text-muted-foreground">{cert.deviceModel}</div>
                        <div className="text-xs text-muted-foreground font-mono">{cert.deviceSerial}</div>
                      </div>
                    </TableCell>
                    <TableCell>
                      <div>
                        <div className="text-sm font-medium text-foreground">{cert.wipeMethod}</div>
                        <div className="text-xs text-muted-foreground">{cert.wipePolicy}</div>
                      </div>
                    </TableCell>
                    <TableCell>
                      <div>
                        <div className="text-sm text-foreground">{formatDate(cert.startTime)}</div>
                        <div className="text-xs text-muted-foreground">
                          Duration: {calculateDuration(cert.startTime, cert.endTime)}
                        </div>
                      </div>
                    </TableCell>
                    <TableCell>
                      <Badge className={getStatusColor(cert.status)}>{cert.status.toUpperCase()}</Badge>
                    </TableCell>
                    <TableCell>
                        ...
                        <Dialog>
                          ...
                          <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto component-border">
                            <DialogHeader>
                              ...
                              <DialogDescription>
                                Data destruction certificate and verification information
                              </DialogDescription>
                            </DialogHeader>
                            {selectedCertificate && <CertificateDetails certificate={selectedCertificate} />}
                          </DialogContent>
                        </Dialog>
                        <Button variant="outline" size="sm" onClick={() => handleDownloadCertificate(cert, "json")}>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <p className="font-mono font-medium text-foreground">{certificate.certificateId}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="font-medium text-foreground">{certificate.operatorId}</p>
              </div>
              <div>
                <label className="text-muted-foreground">Created</label>
                <p className="font-medium text-foreground">{formatDate(certificate.createdAt)}</p>
              </div>
              <div>
                <label className="text-muted-foreground">Signature</label>
                <p className={cn("font-medium", certificate.signatureValid ? "text-success" : "text-destructive")}>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 gap-4 text-sm">
              <div>
                ...
                <p className="font-medium text-foreground">{certificate.deviceName}</p>
              </div>
              <div>
                <label className="text-muted-foreground">Model</label>
                <p className="font-medium text-foreground">{certificate.deviceModel}</p>
              </div>
              <div>
                <label className="text-muted-foreground">Serial Number</label>
                <p className="font-mono font-medium text-foreground">{certificate.deviceSerial}</p>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <Card className="component-border component-border-hover">
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            <div>
              <label className="text-muted-foreground">Wipe Method</label>
              <p className="font-medium text-foreground">{certificate.wipeMethod}</p>
            </div>
            <div>
              <label className="text-muted-foreground">Policy Standard</label>
              <p className="font-medium text-foreground">{certificate.wipePolicy}</p>
            </div>
            <div>
              <label className="text-muted-foreground">Start Time</label>
              <p className="font-medium text-foreground">{formatDate(certificate.startTime)}</p>
            </div>
            <div>
              <label className="text-muted-foreground">End Time</label>
              <p className="font-medium text-foreground">{formatDate(certificate.endTime)}</p>
            </div>
            <div>
              <label className="text-muted-foreground">Duration</label>
              <p className="font-medium text-foreground">
                {calculateDuration(certificate.startTime, certificate.endTime)}
              </p>
            </div>
            <div>
              <label className="text-muted-foreground">Evidence Hash</label>
              <p className="font-mono text-xs text-foreground">{certificate.evidenceHash}</p>
            </div>
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent>
          <div className="flex flex-col md:flex-row gap-6">
            <div className="flex-1">
              ...
              <p className="text-xs text-muted-foreground text-center mt-2">QR Code for certificate verification</p>
            </div>
